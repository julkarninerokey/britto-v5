...
const Marksheet = ({navigation}) => {
  ...
  const [noDataMessage, setNoDataMessage] = useState('No marksheet information is currently available.');
    ...
    const checkForData = async () => {
      ...
      try {
        ...
        if (response?.status === 201) {
          setNoDataMessage(response?.message || 'No marksheet information is currently available.');
          setData([]);
          ...
          setNoDataMessage('No marksheet information is currently available.');
          setData([]);
        }
      } catch (error) {
        setNoDataMessage('No marksheet information is currently available.');
