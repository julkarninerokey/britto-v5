...
const NewEnrollmentModal = ({ isOpen, onClose, navigation, onEnrollmentSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [enrollmentData, setEnrollmentData] = useState(null);
  const [paymentHeads, setPaymentHeads] = useState([]);
  const [selectedExam, setSelectedExam] = useState('');
  const [selectedCourses, setSelectedCourses] = useState([]);
  const [enrollmentFee, setEnrollmentFee] = useState(100);
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      if (paymentResponse.success && paymentResponse.data) {
        ...
        const enrollmentHead = paymentResponse.data.find(head => 
          head.category === 'ENROLMENT' || 
          head.name.toLowerCase().includes('enrollment') ||
          head.name.toLowerCase().includes('enrolment')
        );
        setEnrollmentFee(enrollmentHead ? enrollmentHead.unit_price : 100);
      }
  ...
  const getTotalCost = () => {
    return selectedCourses.length * enrollmentFee;
  };
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast('success', 'Success', response.message || 'Successfully enrolled!');
        ...
        setTimeout(async () => {
          if (applicationId) {
            console.log(`Processing payment for application ID: ${applicationId}`);
              ...
              try {
                ...
                await handleDirectPayment({
                  ...
                  onCancel: () => {
                    ...
                    toast('info', 'Payment Cancelled', 'You can make payment later from the enrollment section.');
                  }
                });
              } catch (error) {
        ...
        toast('error', 'Enrollment Failed', response.message || 'Your enrollment has failed. Please try again.');
      }
      
    } catch (error) {
      console.error('Enrollment error:', error);
      toast('error', 'Error', 'An unexpected error occurred during enrollment');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getSelectedExamData = () => {
    if (!selectedExam || !enrollmentData?.exams) return null;
    return enrollmentData.exams.find((exam, index) => index.toString() === selectedExam);
  };
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="full">
        <Modal.Content maxWidth="100%" maxHeight="100%" borderRadius={0} bg={color.background}>
          <Modal.Header bg={color.background} borderBottomWidth={1} borderColor={color.light} py={3}>
            <Text fontSize="lg" fontWeight="600" color={color.text}>New Enrollment </Text>
          </Modal.Header>
          <Modal.CloseButton />
          <Modal.Body p={4}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <VStack space={3}>
              ...
              <VStack space={2}>
                ...
                >
                  {enrollmentData?.exams?.map((examOption, index) => (
                    <Select.Item
                      key={index}
                ...
                <VStack space={2}>
                  <HStack justifyContent="space-between" alignItems="center">
                    ...
                    <Text fontSize="sm" fontWeight="500" color={color.primary}>
                     ৳{enrollmentFee} x {selectedCourses.length} = {getTotalCost()}/-
                    </Text>
                  </HStack>
                ...
                <VStack space={3}>
                  ...
                  <Box bg={color.light} p={3} borderRadius={8}>
                    <VStack space={2}>
                      ...
                      <HStack justifyContent="space-between" alignItems="center">
                        <VStack>
                          <Text fontSize="xs" color={color.muted}>Enrollment Last Date</Text>
                        </VStack>
                      ...
                      <HStack justifyContent="space-between" alignItems="center">
                        <Text fontSize="sm" color={color.text}>
                          Total Amount (৳{enrollmentFee} x {selectedCourses.length} courses)
                        </Text>
